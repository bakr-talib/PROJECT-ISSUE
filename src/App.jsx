...
import { themes } from "./hooks/useTheme"; // تعديل المسار هنا
...
>
      ...
      {showButtons && (
        <div className="absolute space-y-2">
          ...
          className="border bg-[#2F1C35]  absolute -top-16 right-2 w-7 h-7 rounded-full bg-transparent"></button>
          <button 
          onClick={()=>handleThemeChange("Silver_dark")}
          
          className="border bg-[#6C6E71] absolute -top-10 right-4 w-7 h-7 rounded-full bg-transparent"></button>
          <button 
          onClick={()=>handleThemeChange("Green_dark")}
          
          className="border bg-[#2D6A4F] absolute -top-2 right-1 w-7 h-7 rounded-full bg-transparent"></button>
          <button 
          onClick={()=>handleThemeChange("blue_dark")}
          
          className="border bg-[#0F1933] absolute top-2 -right-7 w-7 h-7 rounded-full bg-transparent"></button>
          <button 
          onClick={()=>handleThemeChange("Yellow_dark")}
          
          className="border bg-[#8A6B1F] absolute top-2 -right-16 w-7 h-7 rounded-full bg-transparent"></button>
        </div>
      )}
    </div>
       <Header setActiveContent={setActiveContent} />
      <div className=" text-[var(--primary-color)]  flex justify-center  items-center md:order-3"
